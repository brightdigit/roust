      ...
      for (var i = 0; i < controllers.length; i++) {
        ...
        for (var resource in resources) {
          if (resources[resource].actions) {
            ...
            for (var subresource in resources[resource]) {
              var fullresource = resource + '/' + subresource;
              var routes = rm(fullresource, router);

              for (var action in resources[resource][subresource].actions) {
                routes(am[action], resources[resource][subresource].actions[action]);
              }
            }
          }
        }
