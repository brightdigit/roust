...
module.exports = function () {
  ...
  Roust.verb = function (action) {

  };

  Roust.routeManager = function (resource, router) {
    var base = '/'+resource;
    this.routes = [
      router.route(base),
  ...
  Roust.routes = function (resource, router) {
    var routes = new Roust.routeManager(resource, router);
    ...
    router : function () {
      ...
      for (var i = 0; i < this._controllers.length; i++) {
        ...
        for (var resource in controllers) {
          var routes = Roust.routes(resource, this._router);
          for (var action in controllers[resource].actions) {
             console.log(action);
            routes(Roust.actionmap[action], controllers[resource].actions[action]);
          }
        }
      };
